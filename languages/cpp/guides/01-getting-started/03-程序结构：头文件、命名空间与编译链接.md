...
在 [Hello World 背后：C 语言程序的执行原理](../../c/guides/01-getting-started/03-Hello World背后：C语言程序的执行原理.md) 中，我们学习了 C 程序从源码到可执行文件的完整流程：预处理 → 编译 → 汇编 → 链接。C++ 程序遵循相同的编译模型，但在**程序组织**上引入了命名空间、更严格的头文件约定等新机制。
...
> 内存布局、目标文件结构、静态/动态链接等细节请参阅 [C 语言第 03 篇](../../c/guides/01-getting-started/03-Hello World背后：C语言程序的执行原理.md)。
...
- [C 语言程序的执行原理](../../c/guides/01-getting-started/03-Hello World背后：C语言程序的执行原理.md)
