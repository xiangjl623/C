...
在 [C 语言运算符与表达式](../../c/guides/02-core-syntax/05-运算符与表达式：程序的计算逻辑.md) 中，我们系统学习了算术、关系、逻辑、赋值、位运算等运算符。C++ 完全兼容这些运算符，并新增了 **new/delete**、**nullptr**、流运算符 **<<**/**>>** 等重要特性。
...
| 类别 | 运算符 | 详见 |
|------|--------|------|
| 算术 | `+` `-` `*` `/` `%` | [C 第 05 篇 §二](../../c/guides/02-core-syntax/05-运算符与表达式：程序的计算逻辑.md) |
| 关系 | `>` `<` `>=` `<=` `==` `!=` | [C 第 05 篇 §三](../../c/guides/02-core-syntax/05-运算符与表达式：程序的计算逻辑.md) |
| 逻辑 | `&&` `\|\|` `!` | 同上 |
| 赋值 | `=` `+=` `-=` 等 | 同上 |
| 位运算 | `&` `\|` `^` `~` `<<` `>>` | 同上 |
...
- [C 语言运算符与表达式](../../c/guides/02-core-syntax/05-运算符与表达式：程序的计算逻辑.md)
