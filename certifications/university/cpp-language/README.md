# 大学 C++ 程序设计期末考试

> 高校 C++ 课程期末备考专题：大纲、复习计划、考点对照与题库入口

**建议先修**：[大学 C 语言期末](../c-language/) · [languages/c/](../../../languages/c/)

## 双链导航

| 内容类型 | 位置 | 入口 |
|----------|------|------|
| 大纲、复习计划、考点表 | 本目录 | 见下方 |
| 题目与参考答案 | languages/cpp/exams/ | [languages/cpp/exams/](../../../languages/cpp/exams/) |
| 系统教程（15 篇） | languages/cpp/guides/ | [languages/cpp/](../../../languages/cpp/) |
| 语法速查（14 主题） | languages/cpp/references/ | [references/](../../../languages/cpp/references/) |
| 示例代码 | languages/cpp/examples/ | [examples/](../../../languages/cpp/examples/) |

## 备考资料

| 资料 | 说明 | 链接 |
|------|------|------|
| 期末考试大纲 | 01～15 篇考点、题型分布 | [期末考试大纲.md](../../../languages/cpp/exams/期末考试大纲.md) |
| 两周复习计划 | 14 天每日任务 | [两周复习计划.md](../../../languages/cpp/exams/两周复习计划.md) |
| 题库总览 | 选择/填空/问答/纠错/编程各 50 题 | [exams/README.md](../../../languages/cpp/exams/README.md) |

## 适用范围

本专题对齐 Dev Handbook [C++ 教程体系](../../../languages/cpp/)（非特定教材版本）。若学校使用《C++ Primer》等教材，可对照本仓库 guides 章节映射复习。

## 考点速览

| 教程 | 核心考点 | 速查 |
|------|----------|------|
| 01～04 入门 | namespace、引用、iostream、auto | [§1～2](../../../languages/cpp/references/C++常用语法汇总.md#1-程序基础) |
| 05～09 核心 | new/delete、重载、string、vector | [§3～7](../../../languages/cpp/references/C++常用语法汇总.md#3-运算符) |
| 10～11 OOP | 类、继承、virtual、多态 | [§8～9](../../../languages/cpp/references/C++常用语法汇总.md#8-类与对象) |
| 12 STL | 模板、vector、map、algorithm | [§10～12](../../../languages/cpp/references/C++常用语法汇总.md#10-模板) |
| 13～15 实战 | RAII、智能指针、文件 I/O | [§13～14](../../../languages/cpp/references/C++常用语法汇总.md#13-文件流) |

## 推荐复习顺序

1. 确认 [C 语言基础](../../../languages/c/) 已掌握（至少指针、函数、结构体）
2. 阅读 [期末考试大纲.md](../../../languages/cpp/exams/期末考试大纲.md)
3. 按 [两周复习计划.md](../../../languages/cpp/exams/两周复习计划.md) 执行
4. 刷 [languages/cpp/exams/](../../../languages/cpp/exams/) 各题型并对照参考答案
5. 薄弱章节回看 [guides/](../../../languages/cpp/guides/) 与 [examples/](../../../languages/cpp/examples/)

## 相关基础

- [C 语言期末专题](../c-language/)
- [数据结构（STL 实现参考）](../../../fundamentals/04-data-structures/)
- [算法入门](../../../fundamentals/05-algorithms/)

## 模板

新建其他考试专题时，可复制 [certifications/_shared/](../../_shared/) 下的模板。
